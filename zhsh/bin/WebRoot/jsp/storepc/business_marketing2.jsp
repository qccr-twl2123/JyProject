...
<html>
    <head>
        ...
        <meta charset="utf-8">
        <base href="<%=basePath%>">
        <link rel="stylesheet" href="css/storepc/business_marketing2.css">
        <script src="js/jquery-1.8.0.min.js"></script>
        <script src="js/storepc/business_marketing2.js"></script>
    </head>
    <body>
    <c:if test="${storeqx.look eq '1'}">
       <div class="parent">
           <div class="d1">
                <p class="d1_p1">
                	积分方式 <span class="d1_sp1">商家根据自身经营状况五选一</span>
                 </p>
            </div>
           <div class="d2">
             <span class="d2_sp1 d2_sp1_active" id="open" ><a  href="storepc_marketing/goIntegral.do?store_id=${storepd.store_id}&type=1">整店送积分设置</a></span>
           ...
           <p class="parent_p1">选择该方式时，会员在商家消费买单时，按照<span class="orange">实际买单金额X整店积分率</span>的方式进行积分返还</p>
           <div class="d3">
             <span class="d3_sp1">积分率</span>
             <select class="d3_set1"> 
           ...
           <div class="d3">
              <label>
                <input type="checkbox" class="d3_ipt1" ${pd.change_type eq '1'?'checked':'' } onclick="showtext()"/>
                <span class="d3_sp2">使用</span>
              </label>
           </div>
           <div class="d3">
	           	<c:if test="${storeqx.add eq '1'}">
		             <span class="yes" onclick="save()">
		              		 确定
		             </span>
	            </c:if>
           </div>
    ...
    <script type="text/javascript">
    	...
    	function save(){
    		...
    		var backRate = $('.d3_set1 option:selected').val();
