...
<html>
    ...
    <body>
     <c:if test="${qx.look eq '1'}">
    	<!-- 内容 -->
       <form action="zhihuiz_store_file/list.do" method="post" name="Form" id="Form">
	       <div class="dangan2_d1">
	         ...
	         <input type="text" value="${pd.content }" placeholder="可输入序列号商家名称注册电话等查询" class="dangan2_d1_ipt1" name="content"/>
		     		...
		     		<c:forEach items="${varList}" var="var" varStatus="vs">
		              <tr>
		                ...
		                <td>
		                	...
		                	<a href="zhihuiz_store_file/goEdit.do?store_file_id=${var.store_file_id}&currentPage=${page.currentPage}&ok=1&content=${pd.content}&province_id=${pd.province_id}&province_name=${pd.province_name}&city_id=${pd.city_id}&city_name=${pd.city_name}&area_id=${pd.area_id}&area_name=${pd.area_name}" target="ifra"><span class="td_sp1">审核</span></a>
 		                	</c:if>
		                </td>
		              </tr>
		            </c:forEach>
       ...
       //获取城市
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					var url = '<%=path%>/zhihuiz_store_file/delete.do?store_file_id='+Id+'&tm='+new Date().getTime();
