...
<html>
    ...
    <c:if test="${qx.look eq '1'}">
    	<form action="<%=basePath%>youxuan/datalistPageGoods.do" name="Form" id="Form" method="post">
 	       <input type="hidden" name="goods_check" id="goods_check" value="2"  />
	              ...
	              <c:forEach items="${goodslist}" var="var" varStatus="vs">
	              	<tr>
		                <td>${var.createtime}</td>
  		                ...
  		                <td>
		                	<c:if test="${qx.edit eq '1'}"><a  href="<%=basePath%>youxuan/goEditGoods.do?youxuangoods_id=${var.youxuangoods_id}&currentPage=${page.currentPage}&goods_check=${pd.goods_check}" >编辑</a></c:if>
		                	<span>|</span>
		                	<c:if test="${qx.add eq '1'}"><a  href="<%=basePath%>youxuan/gosendyouxuan.do?youxuangoods_id=${var.youxuangoods_id}&currentPage=${page.currentPage}" >推送</a></c:if>
		                </td>
		                <td>${var.tuisong_number}</td>
 		             </tr> 
 	              </c:forEach>           
	          </table>
	          <br/>
