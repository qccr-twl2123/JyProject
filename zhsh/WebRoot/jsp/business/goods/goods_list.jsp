						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										<td>${var.GOODS_CATEGORY_ID}</td>
										<td>${var.GOODS_NAME}</td>
										<td>${var.IMAGE_URL}</td>
										<td>${var.MARKET_MONEY}</td>
										<td>${var.RETAIL_MONEY}</td>
										<td>${var.INTEGRAL_RATE}</td>
										<td>${var.STOCK_NUMBER}</td>
										<td>${var.CONSUMPTION_NUMBER}</td>
										<td>${var.GOODS_STATUS}</td>
										<td>${var.SALES_STATUS}</td>
										<td>${var.STARTTIME}</td>
										<td>${var.ENDTIME}</td>
										<td>${var.PROMOTION_TYPE}</td>
								<td style="width: 30px;" class="center">
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<c:if test="${QX.edit == 1 }">
											<a   title="编辑" onclick="edit('${var.goods_id}');"  class="btn btn-mini btn-info"  ><i class="icon-edit"></i></a>
										</c:if>
										<c:if test="${QX.del == 1 }">
											<a   title="删除" onclick="del('${var.goods_id}');"  class="btn btn-mini btn-info"  ><i class="icon-trash"></i> </a>
										</c:if>
										<!--
