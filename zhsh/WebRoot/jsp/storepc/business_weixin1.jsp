...
<body  >
	...
	<div class="cy_alert" style="display:none">    <!-- alert  成员 -->
		<ul class="alert_box">
			...
			<li class="alt_body">
				<ul style="width:94%;height:100%;overflow-y:auto;padding-left:15px;border-top:1px solid #ccc;line-height:1.6;" class="style_scroll_inf chengyuanList">
					<!-- <li style="width:100%;">aaaaaaaaaaa</li> -->
 				</ul>
			</li>
		</ul>
	</div>
	<ul style="width:100%;height:90%;background:#eee;position:relative;">
		<li class="list">
			<ul>
				<!-- 商家logo  名字 -->
				<li class="sj">
			...
			<ul class="friend_liaotian" style=" height: 100%;display:none; ">
				<li class="cont  friend_chuangkou"  style="display:none;"  >
					<ul style="height:100%;"> 
						...
						<li class="input_box">
							<textarea class="style_scroll_inf message" ></textarea>
    ...
    conn.queryRoomMember({
        ...
        success: function (members) {
            for (var o in members) {
        		...
        		var _name=getname(userName);
        		//var img_url="https://www.jiuyuvip.com/FileSave/File/userFile/"+userName+".png";
        		var str="<li style='width:100%;'>"+_name+"</li>";
	...
	if(id == curUserId ){//单聊
		if(("#"+from).length == 0){//陌生人
			...
			var fasong=$(".jieshou").html();
	 		fasong="<ul style='padding:15px 5%;' id='"+serverMsgId+"'  >"+fasong+"</ul>";
...
function sendFriendMessage(obj){
	    ...
	    var msg = new WebIM.message('txt', id);      // 创建文本消息
 	    var sendMessage=$(obj).parent().prev().val();
    	msg.set({
	        msg: sendMessage,                  // 消息内容
...
function selectEmotionImg(e){
	var str=$("#chat_"+nowid).find(".message").val()+$(e).attr("id");
	$("#chat_"+nowid).find(".message").val(str);
}
