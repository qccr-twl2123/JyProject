...
var showChatUI = function () {
	...
	var _name=getname(curUserId);
	login_userEle.innerHTML = _name;
		...
		if (contact.subscription == 'from' || contact.subscription == 'both') {
			...
			if (!isexist) {
				var lielem = $('<li>').attr({
 				...
 				$('<img>').attr({ "src" : "https://www.jiuyuvip.com/FileSave/File/userFile/"+contact.name+".png","width":"40px","height":"40px"}).appendTo(lielem);//头像
 				var _name=getname(contact.name);
 				$('<span>').html(_name).appendTo(lielem);//名称
	...
	for (i = 0; i < roster.length; i++) {
			   ...
			   if (userName in cache) {
 			   ...
 			   cache[userName] = true;
				 ...
				 //好友列表
				$('<img>').attr({"src": "https://www.jiuyuvip.com/FileSave/File/userFile/"+userName+".png","width":"40px","height":"40px"}).appendTo(lielem);
				//alert(userName);
				var _name=getname(userName);
 				$('<span>').html(_name).appendTo(lielem);
 				$('#contactlistUL').append(lielem);
   	    //-------------------------------------------------
    	}
...
var buildListRoomDiv = function(contactlistDivId, rooms, type) {
	...
	for (i = 0; i < rooms.length; i++) {
		...
		$('<img>').attr({ 'src' : 'static/img/head/group_normal.png'
		}).appendTo(lielem);
...
var showContactChatDiv = function(chatUserId) {
	...
	if (chatUserId.indexOf(groupFlagMark) >= 0) {
		dispalyTitle = "群组<span style='color:blue;font-size:20px;'>" + $(contactLi).attr('displayname') + "</span>聊天中";
		...
		var _name=getname(chatUserId);
 		dispalyTitle = "与<span style='color:blue;font-size:20px;'>" + _name + "</span>聊天中";
			...
			if (rooms) {
				for (i = 0; i < rooms.length; i++) {
					...
					if (existRoom && existRoom.length == 0) {
						...
						$('<img>').attr({ 'src' : 'static/img/head/group_normal.png' }).appendTo(lielem);
						$('<span>').html(roomsName).appendTo(lielem);
...
var appendMsg = function(who, contact, message, onlyPrompt) {
	...
	//发送聊天消息
	var _name=getname(who);
	var headstr = onlyPrompt ? ["<p1 style='    font-size: 20px;'>" + message + "</p1>"] : [ "<p1 style='    font-size: 20px;'>" + _name + "   <span></span>" + "   </p1>",
			"<p2 style='    font-size: 20px;'>" + getLoacalTimeString() + "<b></b><br/></p2>" ];
...
var removeFriendDomElement = function(userToDel, local) {
	...
	if (curChatUserId != userToDel) {
		...
		if (bothRoster.length > 0) {
 		...
 		$('#talkTo').html("<img src=\'https://www.jiuyuvip.com/FileSave/File/userFile/"+userName+".png\' \"  width=\"30px\" height=\"30px\"/> ");
		//未
	}
};
//清除聊天记录
     ...
     <div class="content" id="content" style="display: none">
        <div class="leftcontact" id="leftcontact">
            <div id="headerimg" class="leftheader">
                <span> 
                	<img src="https://www.jiuyuvip.com/FileSave/File/userFile/${pd.store_id}.png" alt="logo" class="img-circle" width="60px" height="60px"
                    style="margin-top: -40px; margin-left: 20px" />
                </span> 
                <span id="login_user" class="login_user_title"  style="widnth:10%"> 
                    <a class="leftheader-font" ></a>
                </span> 
                <!-- <span style="    display: inline-block; margin-left: 100px;">
                    <div class="btn-group" style="margin-left: 5px;" style="display:none;">
                        <button class="btn btn-inverse dropdown-toggle" data-toggle="dropdown">
                            <span class="caret"></span>
                        </button>
                        <ul class="dropdown-menu" >
                            <li onclick="showAddFriend()">添加好友</li>
                            <li onclick="showDelFriend()">删除好友</li>
            ...
            <div id="contractlist11">
                <div class="accordion" id="accordionDiv">
					...
					<div id='em-cr' class="accordion-group"  style="display:none">
                        <div class="accordion-heading">
       ...
       <!--  <div id="rightTop" style="height:45px;"></div> -->
        <!-- 聊天页面 -->
        <div class="chatRight">
            <div id="chat01">
            ...
            <div class="chat02">
                <div class="chat02_title">
					...
					<a style='display:none' id='sendPicBtn' class="chat02_title_btn ctb03" title="选择图片" type='img' ></a>
					<input id='sendAudioInput' style='display:none'/>
					<a style='display:none' id='sendAudioBtn' class="chat02_title_btn ctb02" title="选择语音"  type='audio'></a>
					<input id='sendFileInput' class='emim-hide'/>
					<a style='display:none' id='sendFileBtn' class="chat02_title_btn ctb04" title="选择文件"></a> -->
        ...
        <div id="confirm-block-div-modal" class="modal hide"
            ...
            <div class="modal-body">
                <ul id="confirm-block-footer-body"></ul>
            </div>
            <div class="modal-footer">
                <button id="confirm-block-footer-cancelButton" class="btn" data-dismiss="modal">关闭</button>
            </div>
        </div>
