...
<div class="select_box">
	<div class="sel_cont">
		...
		<div class="sel_anniu">
			<a class="sel_button sel_jrgwc" onclick="addShopCart()">加入购物车</a>
			<a class="sel_button sel_nowbay" onclick="nowbuy()">立即购买</a>
		</div>
	</div>
</div>
<!-- 顶部条 -->
  ...
  <div class="swiper-wrapper">
    <div class="swiper-slide">
		...
		<div class="sj_inf" id="text_des">
			<!-- <p><span>销售商家：</span><span>金溪测试11111</span></p>
			<p><span>提货地址：</span><span>测试地址测试地址测试地址测试地址</span></p>
			<p><span>积分率：</span><span>10%</span><span>,提货后可获赠<span>0.00</span>积分</span></p> -->
		</div>
		<div class="dianpu">
			<div class="dp_img">
...
<footer>
	...
	<a href="<%=basePath%>html_member/goMyYouXuanShopCart.do"   class="foot_guc"><span class="iconfont icon-gouwuche1 icon2"></span><span  class="foot_font_sp">商品将保留</span><span class="bl_djs shopcarttime"></span></a>
...
/* 	    	async: false,*/	
     		success: function(data){
		   	 	if(data.result == "1"){
		   	 		...
		   	 		// 修改分享内容
		   	 		var  str=goodspd.store_name+"|"+goodspd.goods_name+"|仅需:"+goodspd.bp_salemoney+"元,低至"+goodspd.goods_zkrate+",数量有限，立即抢购。";
		   	 		document.title=str;
		   	 		var metastr="<meta type='description' content="+str+"> ";
		   	 		var img_str="<div style='display:none'><img src="+goodsgglist[g].gg_imageurl+" style='width:300px;height:300px;''></div>"
		   	 		$("head").append(metastr);
		   	 		$("body").prepend(img_str)
...
//新增收藏和取消收藏
$(".dp_sc a").click(function(){
 	if("${pd.member_id}" != ""){
 		if($(".dp_sc a")[0].innerText=="收藏"){
 			$(".dp_sc a").css({"background":"#ccc"})
 			$(".dp_sc a")[0].innerText="取消收藏"
 		}else if($(".dp_sc a")[0].innerText=="取消收藏"){
 			$(".dp_sc a").css({"background":"#f60008"})
 			$(".dp_sc a")[0].innerText="收藏"
 		}
		$.ajax({
		  ...
		  var r=confirm("请先前往登陆。")
			  if (r==true){ 
			  	// 确定点击事件
			      window.location="https://www.jiuyuvip.com/html_member/gouhtmlLogin.do"
			    }
			  else{
			  	// 取消点击事件
		...
		$.ajax({
	        ...
	        url:'<%=basePath%>app_goods/caozuoShopCar.do', 
	  	 	data:{
	  	 		...
	  	 		"goods_type":"2",
	  	 		"caozuo_number":$("#shopnumber").html().trim(),
	  	 		"member_id":"${pd.member_id}"
		  	 	},                
	        dataType:"json",
	        success: function(data){
	        	...
	        	if(data.result == "1"){
	        		alert("恭喜你，成功添加进购物车^_^....");
	        		lasttime=30*60*1000;
	        		$("#shopnumber").html("1");
	  	        	$("#gg_kucun").html(number);
		        	$("."+youxuangg_id).attr("needsale_number",number+"");
	       	 		needsale_number=number;
 	       	 	}else{
 	       	 		alert(data.message);
 	       	 	}
 	         }
...
function nowbuy(){
	...
	if(youxuangg_id == ""){
		...
		var shopnumber=$("#shopnumber").html().trim();
		if(parseInt(shopnumber) <= parseInt(needsale_number) ){
			$.ajax({
		        type:"post",
		        ...
		        success: function(data){
		        	//判断是否还有库存
  		        	if(data.result == "1"){
  		        		window.location.href="<%=basePath%>html_member/goReadyPayOrder.do?goods_id=${pd.youxuangoods_id}&gopay_type=1&goodsinfor="+youxuangg_id+"@"+shopnumber;
 	 	       	 	}else{
 	 	       	 		alert(data.message);
 	 	       	 	}
 		         }
...
　　var windouhei=$(window).height()
	...
	// $('.banner img').css({"height":windowid})
