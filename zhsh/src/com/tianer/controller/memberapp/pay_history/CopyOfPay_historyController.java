//package com.tianer.controller.app.pay_history;
//
//
...
//public class CopyOfPay_historyController extends BaseController {
//	
...
//					try{
//						pd = this.getPageData();
//						String pay_type=pd.getString("pay_type");//1-现金支付，2-第三方支付，3-提货卷（第三方支付）
...
//									map.put("result", "0");
//									map.put("message", "支付金额不能小于"+df.format(Double.parseDouble(fourbackintegral_integral)*2));
//									map.put("data", "");
//							    	return map;
//							}
//		  				}
...
//						double isnow_wealth=Double.parseDouble(appStoreService.sumStoreWealth(pd));
...
//						}
//						//1116新加
//						pd.put("discount_after_money", df.format(Double.parseDouble(sale_money)-Double.parseDouble(discount_money)));
...
// 						pd.put("order_tn", order_id);
// 						//----------------
//						double ac=Double.parseDouble(actual_money);
//						System.out.println("开始新增订单==============================pay_type="+pay_type);
//						if(pay_type.equals("1")){
//							System.out.println("暂无现金支付");
//						}else if(pay_type.equals("2")){
//							 System.out.println("优惠买单支付....");
//							 if(ac == 0){
//								    pd.put("in_jiqi", "1");
// 								    map2=createOrder(pd,mpd,pd);//调订单支付的接口
//								    map1.put("charge", "");
//								    if(map2.get("result").equals("0")){//支付失败
//										    	result=(String) map2.get("result");
...
//									System.out.println("==============================新增订单结束");
//									//2.获取charge对象
//									System.out.println("==============================获取charge对象");
//									Charge charge = ChargeExample.getPay(order_id, Double.parseDouble(actual_money)*100,ip,pay_way,"1",2);
//									if(charge == null ){
//		 								map.put("result", "0");
...
//										e=payapp_historyService.tihuoByOrderId(e);
//										if(e==null){
//											istihuo=false;
...
//	 			   						moneypd.put("content", appStoreService.findById(pd).getString("store_name")+"消费抵用");
...
//		 			   				System.out.println("修改使用红包的状态========================redpackage_id="+redpackage_id);
...
//		 			   				}
//									pd.put("order_status", "1");//支付完成完成
//	  								pd.put("tihuo_status", "0"); 
//	  								pd.put("remark", "");
//	 								pd.put("channel", pay_way);
// 									appOrderService.saveOrder(pd);
// 									System.out.println("开始生成总后台记录");
//				 			   		PageData waterpd=new PageData();
//			 	    				waterpd.put("pay_status","97");
//			 	    	   			waterpd.put("waterrecord_id", order_id);
//			 	   					waterpd.put("user_id", mpd.getString("member_id"));
//			 	   					waterpd.put("user_type", "2");
//			 	    				waterpd.put("withdraw_rate","0");
//			 	   					waterpd.put("money_type","2");
//			 	   	 				waterpd.put("money", sale_money);
//			 	   	 				waterpd.put("reduce_money",discount_money);
//			 	   					waterpd.put("arrivalmoney", df.format(Double.parseDouble(sale_money)-Double.parseDouble(discount_money)));
// 			 	   					waterpd.put("nowuser_money", df.format(Double.parseDouble(ServiceHelper.getAppMemberService().findById(mpd).getString("now_money"))-Double.parseDouble(user_balance)));
//			 	    				waterpd.put("application_channel", "1"); 
//			 	   					if(pay_way.contains("alipay")){
//			 	   						waterpd.put("remittance_type","3" );
//			 	    	 				waterpd.put("alipay_money",actual_money );
//			 	    	 				waterpd.put("remittance_name",Const.payjiqi[3] );
//			 	   					}else if(pay_way.contains("wx")){
//			 	   						waterpd.put("remittance_type","4" );
//			 	   						waterpd.put("wx_money",actual_money );
//			 	   						waterpd.put("remittance_name",Const.payjiqi[4] );
//			 	   					}else if(pay_way.contains("nowpay")){
//			 	   						waterpd.put("remittance_type","2" );
//			 	   						waterpd.put("nowypay_money",actual_money);
//			 	   						waterpd.put("remittance_name",Const.payjiqi[0] );
//			 	   					}else if(pay_way.contains("pple")){
//			 	   						waterpd.put("remittance_type","5" );
//			 	   						waterpd.put("apple_money",actual_money);
//			 	   						waterpd.put("remittance_name",Const.payjiqi[5] );
//			 	   					}else{
//			 	   						waterpd.put("remittance_type","5" );
//			 	   						waterpd.put("bank_money",actual_money);
//			 	    					waterpd.put("remittance_name",Const.payjiqi[1] );
//			 	   					}
//			 	   					waterpd.put("integral_money",user_integral); 
//			 	   					waterpd.put("balance_money",user_balance); 
...
//			 	    				waterpd.put("payee_number",pd.getString("store_id"));
//			 	     	 			waterpd.put("order_tn", order_id);
...
//								//2.获取charge对象
//								System.out.println("==============================获取charge对象");
//								Charge charge = ChargeExample.getPay(order_id, Double.parseDouble(actual_money)*100,ip,pay_way,"1",1);
//								if(charge == null ){
//	 								map.put("result", "0");
//									map.put("message", "支付失败，charge出错");
//									map.put("data", "");
...
//		try{
//						pd = this.getPageData();
//						String waterrecord_id=BaseController.getCZUID();//支付历史记录
// 						PageData mpd=new PageData();
//			 			mpd=appMemberService.findById(pd);//用户详情
//			 			if(mpd == null){
//			 				map.put("result", "0");
...
//					 */
//					//2.获取charge对象
// 					Charge charge = ChargeExample.getPay(waterrecord_id, Double.parseDouble(money)*100,ip,pay_way,"2",2);
//					if(charge == null ){
//						result="0";
...
// 		public void payBackWay(HttpServletRequest request, HttpServletResponse response) throws Exception {
...
//									            String body=jv.optString("body");//1-订单支付(会员)，2-充值（商家/会员）,3-扫码支付
//						 			            int sub=jv.optInt("subject");//1-商家，2-会员,3-服务商,4-业务员，5-商家支付保证金
...
//												 	    				waterpd.put("withdraw_rate","0");
//												 	   					waterpd.put("money_type","2");
//													 	   				waterpd.put("money", pd.getString("sale_money"));
//												 	   	 				waterpd.put("reduce_money",pd.getString("discount_money"));
//												 	   					waterpd.put("arrivalmoney", df.format(Double.parseDouble(pd.getString("sale_money"))-Double.parseDouble(pd.getString("discount_money"))));
//												 	   					waterpd.put("nowuser_money", ServiceHelper.getAppMemberService().findById(mpd).getString("now_money"));
//												 	    				waterpd.put("application_channel", pd.getString("in_jiqi")); 
//												 	   					if(pd.getString("channel").contains("alipay")){
//												 	   						waterpd.put("remittance_type","3" );
//												 	    	 				waterpd.put("alipay_money",pd.getString("actual_money") );
//												 	    	 				waterpd.put("remittance_name",Const.payjiqi[3] );
//												 	   					}else if(pd.getString("channel").contains("wx")){
//												 	   						waterpd.put("remittance_type","4" );
//												 	   						waterpd.put("wx_money",pd.getString("actual_money") );
//												 	   						waterpd.put("remittance_name",Const.payjiqi[4] );
//												 	   					}else if(pd.getString("channel").contains("nowpay")){
//												 	   						waterpd.put("remittance_type","2" );
//												 	   						waterpd.put("nowypay_money",pd.getString("actual_money") );
...
//												 	   						waterpd.put("remittance_type","5" );
//												 	   						waterpd.put("apple_money",pd.getString("actual_money") );
//												 	   						waterpd.put("remittance_name",Const.payjiqi[5] );
//												 	   					}else{
//												 	   						waterpd.put("remittance_type","5" );
//												 	   						waterpd.put("bank_money",pd.getString("actual_money"));
//												 	    					waterpd.put("remittance_name",Const.payjiqi[1] );
//												 	   					}
//													 	   				waterpd.put("integral_money",user_integral); 
//												 	   					waterpd.put("balance_money",user_balance); 
...
//												 	    				ServiceHelper.getWaterRecordService().saveWaterRecord(waterpd);
//												 	    				waterpd=null;
//												 	    				System.out.println("更新订单状态结束========================提货卷");
//  											 			       		}else if(sub == 2){ 
...
//							 			            		}
//  								 			       		}else if(sub == 5){ 
//  								 			       			String sj_money=df.format(price/100);
//								 			       			System.err.println("优选第三方支付===========================");
...
//																	orderpd.put("channel", channel);
//																	orderpd.put("order_tn", trade);
//										 			   				appOrderService.editStatus(orderpd);
//										 			   				//订单详情
//										 			   				orderpd=appOrderService.findById(orderpd);
//										 			   				if(orderpd != null){
//											 			   				orderpd.put("member_id", orderpd.getString("payer_id"));
//											 			   				//用户详情
//	 										 			            	PageData mpd=new PageData();
//												 			       		mpd=appMemberService.findById(orderpd);
//												 			       		//
//	 											 			   			System.out.println("开始生成总后台记录");
//													 			   		PageData waterpd=new PageData();
//												 	    				waterpd.put("pay_status","97");
...
//												 	   					waterpd.put("arrivalmoney", df.format(Double.parseDouble(orderpd.getString("sale_money"))-Double.parseDouble(orderpd.getString("discount_money"))));
//												 	   					waterpd.put("nowuser_money", ServiceHelper.getAppMemberService().findById(mpd).getString("now_money"));
//												 	    				waterpd.put("application_channel", orderpd.getString("in_jiqi")); 
//												 	   					if(orderpd.getString("channel").contains("alipay")){
//												 	   						waterpd.put("remittance_type","3" );
//												 	    	 				waterpd.put("alipay_money",sj_money );
//												 	    	 				waterpd.put("remittance_name",Const.payjiqi[3] );
//												 	   					}else if(orderpd.getString("channel").contains("wx")){
//												 	   						waterpd.put("remittance_type","4" );
//												 	   						waterpd.put("wx_money",sj_money );
//												 	   						waterpd.put("remittance_name",Const.payjiqi[4] );
//												 	   					}else if(orderpd.getString("channel").contains("nowpay")){
//												 	   						waterpd.put("remittance_type","2" );
//												 	   						waterpd.put("nowypay_money",sj_money );
...
//												 	   						waterpd.put("remittance_type","5" );
//												 	   						waterpd.put("apple_money",sj_money );
//												 	   						waterpd.put("remittance_name",Const.payjiqi[5] );
//												 	   					}else{
//												 	   						waterpd.put("remittance_type","5" );
//												 	   						waterpd.put("bank_money",sj_money);
//												 	    					waterpd.put("remittance_name",Const.payjiqi[1] );
//												 	   					}
...
//		 		 			 			       			logger.info("更新商家财富信息====================================");
//						 			   					spd.put("wealth_type", "1");//1-积分，2-金钱
//							 			   				PageData moneypd=appStoreService.findWealthById(spd);
//							 			   				double now_wealth=Double.parseDouble(moneypd.getString("now_wealth"));
//							 			   			    double m=price/100;
//							 			   				moneypd.put("now_wealth", df.format(now_wealth+m));
// 							 			   				appStoreService.editWealthById(moneypd);//更新财富信息
...
// 							 		    				waterpd.put("withdraw_rate","0");
// 							 		   					waterpd.put("money_type","1");
// 							 		   	 				waterpd.put("money", df.format(m));
// 							 		   	 				waterpd.put("reduce_money","0");
// 							 		   					waterpd.put("arrivalmoney",  df.format(m));
// 							 		   					waterpd.put("nowuser_money","0");
// 							 		   					waterpd.put("application_channel", historypd.getString("in_jiqi"));
// 							 			   				if(channel.contains("wx")){
// 							 				   				waterpd.put("remittance_name",Const.payjiqi[4] );
// 							 			   					waterpd.put("remittance_type","4" );
// 							 			   					waterpd.put("wx_money",  df.format(m) );
// 							 							}else if(channel.contains("alipay")){
// 							 								waterpd.put("remittance_name", Const.payjiqi[3]);
// 							 			   					waterpd.put("remittance_type","3" );
// 							 			   					waterpd.put("alipay_money",  df.format(m) );
// 							 							}else{
// 							 								waterpd.put("remittance_name", Const.payjiqi[3]);
// 							 			   					waterpd.put("remittance_type","1" );
// 							 			   					waterpd.put("bank_money",  df.format(m) );
// 							 							}
...
//						 						 					    TongYong.complex_scoreAdd(spd.getString("store_id"), complex_score+"");
//								 	 			   			}
//								 						}
//								 						//充值第二次时进入
...
//		  	 		 			            } else if(sub ==2){//会员
//					 		 			            //新增用户余额充值记录
//		  	 		 			            				logger.info("用户余额充值开始===pay_history_id="+orderno);
//			  	 		 			            			PageData waterpd=new PageData();
//			  			  	 			            		waterpd.put("waterrecord_id", orderno);
...
//					 			 			       			System.out .println("用户详情======================"+mpd);
//		 				 		 			    			double now_money=Double.parseDouble(mpd.getString("now_money"));
//		 				 		 			  				double m=price/100;
//		 				 		 			  				System.out .println("新增会员充值历史信息=========================");
//		 				 		 			  				PageData moneypd=new PageData();
...
//				 		 			  						moneypd.put("number", df.format(m));
//				 		 			  						moneypd.put("now_money", df.format(now_money+m));
...
// 			  	 			 			   	    wpd.put("jiaoyi_id", orderno+"1");
//			  	 				 			    wpd.put("profit_type", "2");
//			  	 				 			    wpd.put("pay_type", channel);
//			  	 				 			    wpd.put("number",df.format( m));
...
//			  	 				 			    wpd.put("last_wealth", df.format(Double.parseDouble(appStoreService.sumStoreWealth(spd))));
//			  	 				 			    wpd.put("store_wealthhistory_id", orderno+"1");
// 			  	 					   			appStoreService.saveWealthhistory(wpd);
//	  		  	 								System.out.println("更新商家历史消息处理状态======================="+historypd);
//			  	 								historypd.put("process_status", "1");
// 			  	 		 						historypd.put("user_id", Const.jiuyunumber);
// 			  	 		 						appStoreService.editWealthHistoryStatus(historypd);
// 			  	 				       			//更新保证金状态
...
//			  	 								if(cpd != null){
//			  	 									String clerk_commission=cpd.getString("clerk_commission");
//			  	 									if(clerk_commission == null || clerk_commission.equals("")){
//			  	 										clerk_commission="0";
//			  	 									}
...
//			  	 									}
//			  	 						 			sp_money+=m*Double.parseDouble(service_provider_commission)/100;//服务费
//			  	 						 			clerk_money+=m*Double.parseDouble(clerk_commission)/100;//服务费
//			  	 								}
//			  	 	 					 		PageData sppd=sp_fileService.findById(spd);
//			  	 						 		if(sppd != null){
//			  	 						 			sppd.put("nowmoney", df.format(Double.parseDouble(sppd.getString("nowmoney"))+sp_money));
//			  	 							 		sp_fileService.edit(sppd);
//			  	 							 		//添加一个支付记录
//			  	 				 	 				PageData mmpd=new PageData();
//			  	 				 	 				mmpd.put("profit_name", sppd.getString("team_name"));//收益对象
//			  	 				 	 				mmpd.put("yewu_name", spd.getString("store_name"));//业务对象
//			  	 				 	 				mmpd.put("yewu_id", spd.getString("store_id"));//业务对象
//			  	 					 	 			mmpd.put("yewu_type", "1");//业务对象
//			  	 				 	 				mmpd.put("money", df.format(sp_money));//金额
...
//			  	 							 		PageData clepd=clerk_fileService.findById(spd);
//			  	 							 		if(clepd != null){
//			  	 							 			clepd.put("nowmoney", df.format(Double.parseDouble(clepd.getString("nowmoney"))+clerk_money));
//			  	 							 			clerk_fileService.edit(clepd);
//			  	 							 			//添加一个提现记录
//			  	 								 		mmpd=null;
//			  	 					 	 				mmpd=new PageData();
//			  	 					 	 				mmpd.put("profit_name", clepd.getString("clerk_name"));//收益对象
//			  	 					 	 				mmpd.put("yewu_name", spd.getString("store_name"));//业务对象
//			  	 					 	 				mmpd.put("yewu_id", spd.getString("store_id"));//业务对象
//			  	 						 	 			mmpd.put("yewu_type", "1");//业务对象
//			  	 					 	 				mmpd.put("money", df.format(clerk_money));//金额
//			  	 					 	 				mmpd.put("money_type", "1");//1、销售提成: 2、积分收益： 3、平台奖励 
//			  	 						 	 			mmpd.put("operate_type", "2"); //1-服务商，2-业务员
//			  	 					 	 				mmpd.put("operate_id", clepd.getString("clerk_file_id")); 
//			  	 					 	 				mmpd.put("isshouyi", "0");//0-收益，1-消费
//			  	 					 	 				mmpd.put("isjihuo", "1");//0-未激活，1-已激活
//			  	 					 	 				mmpd.put("service_performance_id", orderno);//收益对象
//			  	 					 	 				service_performanceService.save(mmpd);
//			  	 							 		}
//			  	 	 						 		spd.put("sp_money", df.format(sp_money));
//			  	 							 		spd.put("clerk_money", df.format(clerk_money));
//			  	 							 		System.out.println("服务商以及业务员的提成"+spd.getString("sp_money")+"**"+spd.getString("clerk_money"));
//			  	 								 	//设置定时器
...
// 	 				 			   			PageData waterpd=new PageData();
//	 					    				waterpd.put("pay_status","1");
//	 					    	   			waterpd.put("waterrecord_id",orderno+"1");
//	 					   					waterpd.put("user_id", spd.getString("store_id"));
//	 					   					waterpd.put("user_type", "1");
//	 					    				waterpd.put("withdraw_rate","0");
//	 					   					waterpd.put("money_type","1");
...
//	  	 											zspd.put("last_wealth", df.format(Double.parseDouble(appStoreService.sumStoreWealth(spd))));
//	  	 											zspd.put("store_wealthhistory_id", "ZS"+orderno);
//	 			  	 					   			appStoreService.saveWealthhistory(zspd);
//	 			  	 					   			zspd=null;
//		 			  	 					   		System.out .println("新增充值记录tb_pay_history========================");
//		 	 				 			   			waterpd=new PageData();
//			 					    				waterpd.put("pay_status","1");
//			 					    	   			waterpd.put("waterrecord_id", "ZS"+orderno);
//			 					   					waterpd.put("user_id", spd.getString("store_id"));
//			 					   					waterpd.put("user_type", "1");
//			 					    				waterpd.put("withdraw_rate","0");
//			 					   					waterpd.put("money_type","1");
...
//											System.out.println(e.toString()+"商家支付服务费");
//									}
//		  	 			            System.out .println("商家支付服务费处理结束========================");
// 		  	 			        }
// 			 	 			     
//		 			        } 
////		   	 			      else if(body.equals("3")){//订单扫码支付
////		 			        			System.out.println("****************进入扫码支付***********************修改订单状态");
////		 			        			String order_id=orderno.substring(6);
////					        			//修改订单状态
////		 			        			pd.put("order_id", order_id);
////				 			   			pd.put("order_status", "1");
////				 			   			logger.info("修改订单状态");
...
////					 			   		pd.put("number", Const.complexscore[1]);//更新商家的综合分值
////					 					storeService.editComplexscore(pd);
////					        } 
//		   	 			      else if(body.equals("4")){//优选商品的支付
...
//		 			       			PageData sppd=sp_fileService.findById(spd);
//	 						 		if(sppd != null){
//	 						 			sppd.put("nowmoney", df.format(Double.parseDouble(sppd.getString("nowmoney"))+m));
//	 							 		sp_fileService.edit(sppd);
//	 							 		//添加一个支付记录
//	 				 	 				PageData mmpd=new PageData();
//	 				 	 				mmpd.put("profit_name", sppd.getString("team_name"));//收益对象
...
////			 DecimalFormat    df   = new DecimalFormat("######0.00"); 
////			 String orderno="201612020229508714";//支付记录号
////			 int sub=2;
