package com.tianer.controller.business.menu_qx;
 ...
 */
@Controller
@RequestMapping(value="/menu_qx")
public class Menu_qxController extends BaseController {
	...
	public ModelAndView save() throws Exception{
		...
		pd = this.getPageData();
		pd.put("MENU_QX_ID", this.get32UUID());	//主键
	...
	public ModelAndView list(Page page){
		...
		try{
			...
			page.setPd(pd);
			List<PageData>	varList = menu_qxService.list(page);	//列出Menu_qx列表
			this.getHC(); //调用权限
			mv.setViewName("business/menu_qx/menu_qx_list");
	...
	public ModelAndView goAdd(){
		logBefore(logger, "去新增Menu_qx页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			mv.setViewName("business/menu_qx/menu_qx_edit");
	...
	public ModelAndView goEdit(){
		...
		pd = this.getPageData();
		try {
			pd = menu_qxService.findById(pd);	//根据ID读取
			mv.setViewName("business/menu_qx/menu_qx_edit");
