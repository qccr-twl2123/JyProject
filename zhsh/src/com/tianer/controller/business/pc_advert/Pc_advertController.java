package com.tianer.controller.business.pc_advert;
 ...
 */
@Controller
@RequestMapping(value="/pc_advert")
public class Pc_advertController extends BaseController {
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, "新增Pc_advert");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		pd.put("PC_ADVERT_ID", this.get32UUID());	//主键
		pc_advertService.save(pd);
	...
	public void delete(PrintWriter out){
		logBefore(logger, "删除Pc_advert");
		PageData pd = new PageData();
		try{
			pd = this.getPageData();
			pc_advertService.delete(pd);
			out.write("success");
			out.close();
		} catch(Exception e){
	...
	public ModelAndView list(Page page){
		...
		try{
			...
			page.setPd(pd);
			List<PageData>	varList = pc_advertService.list(page);	//列出Pc_advert列表
			this.getHC(); //调用权限
			mv.setViewName("business/pc_advert/pc_advert_list");
	...
	public ModelAndView goEdit(){
		logBefore(logger, "去修改Pc_advert页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			pd = pc_advertService.findById(pd);	//根据ID读取
			mv.setViewName("business/pc_advert/pc_advert_edit");
