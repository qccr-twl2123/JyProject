...
public class ObjectExcelRead {
				...
				for (int j = startcol; j < cellNum; j++) {				//列循环开始
					...
					String cellValue = null;
  					if (null != cell) {
						...
						switch (n) { 					// 判断excel单元格内容的格式，并对其进行转换，以便插入数据库
						case 0:
	 ...
	 * @return list
	 */
	public static List<Object> readExcelByXLS(MultipartFile file, int startrow, int startcol, int sheetnum) {
		List<Object> varList = new ArrayList<Object>();
		CommonsMultipartFile cf= (CommonsMultipartFile)file; 
        DiskFileItem dfi = (DiskFileItem)cf.getFileItem(); 
        File target = dfi.getStoreLocation();
		try {
			FileInputStream fi = new FileInputStream(target);
			//HSSFWorkbook wb = new HSSFWorkbook(fi);
